...
- A **role context** (`apps/web/lib/role.ts` + a small client provider): resolves the current demo role from the existing `valor_demo_auth` cookie (extended to carry a role, defaulting to `owner`), exposed via a hook. No real auth change — demo-only, mirrors the existing placeholder gate.
